...
export default function PaymentMethodModal({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div
                className="bg-white rounded-2xl shadow-xl w-full max-w-[340px] sm:max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200"
                role="dialog"
                aria-modal="true"
            >
                <div className="bg-red-600 px-5 py-3.5 sm:px-6 sm:py-4 flex justify-between items-center rounded-t-2xl">
                    <h3 className="text-base sm:text-lg font-bold text-white">Select Payment Method</h3>
                    <button
                        onClick={onClose}
                ...
                <div className="p-5 sm:p-6 space-y-3 sm:space-y-4">
                    ...
                    <label
                        className={`
              flex items-start p-3 sm:p-4 border rounded-xl cursor-pointer transition-all duration-200
                    ...
                    >
                        <div className="flex items-center h-6">
                            <input
                                ...
                                name="paymentMethod"
                                value="wallet"
                                className="w-4 h-4 sm:w-5 sm:h-5 text-red-600 bg-gray-100 border-gray-300 focus:ring-red-500 cursor-pointer"
                        ...
                        <div className="ml-3 sm:ml-4 flex-1">
                            <div className="flex items-center gap-2">
                                <Wallet className={`w-4 h-4 sm:w-5 sm:h-5 ${selectedMethod === 'wallet' ? 'text-red-600' : 'text-gray-500'}`} />
                                <span className="block text-xs sm:text-sm font-semibold text-gray-900">
                                    Wallet Balance (Saldo Jualin)
                                </span>
                            </div>
                            <span className={`block text-xs sm:text-sm mt-1 ${isWalletSufficient ? 'text-gray-500' : 'text-red-500'}`}>
            ...
            `}
                    >
                        ...
                        <div className="ml-3 sm:ml-4 flex-1">
                            <div className="flex items-center gap-2">
                                <CreditCard className={`w-4 h-4 sm:w-5 sm:h-5 ${selectedMethod === 'gateway' ? 'text-red-600' : 'text-gray-500'}`} />
                                <span className="block text-xs sm:text-sm font-semibold text-gray-900">
                                    Payment Gateway
                                </span>
                            </div>
                ...
                <div className="p-5 sm:p-6 bg-gray-50 border-t border-gray-100">
                    <div className="flex justify-between items-end mb-4">
                        <span className="text-gray-600 text-xs sm:text-sm font-medium">Product price:</span>
                        <span className="text-lg sm:text-xl font-bold text-gray-900">{formatCurrency(productPrice)}</span>
                    </div>
                    <button
                        onClick={handleConfirm}
                        className="w-full text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 font-bold rounded-xl text-sm sm:text-md px-5 py-3 sm:py-3.5 text-center shadow-md transition-all duration-200 transform hover:-translate-y-0.5"
                    >
                        Pay Now
                    </button>
                </div>

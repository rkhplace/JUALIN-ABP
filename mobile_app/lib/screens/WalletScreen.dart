...
class _WalletScreenState extends State<WalletScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final history = await _paymentService.getPurchaseHistory(); // fallback untuk list transaksi
      if (!mounted) return;
      setState(() {
  ...
  String _formatCurrency(dynamic amount) {
    int val = 0;
    if (amount is num) val = amount.toInt();
    else if (amount is String) val = double.tryParse(amount)?.toInt() ?? 0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Saldo & Transaksi'), backgroundColor: Colors.white, foregroundColor: Colors.black),
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(_formatCurrency(_balance), style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 12),
                        ElevatedButton(
                          onPressed: () {}, // tambahkan aksi tarik saldo jika ada screen
                  ...
                  const SizedBox(height: 8),
                  ..._transactions.map((t) {
                    final title = t['first_item_name'] ?? 'Order #${t['order_id'] ?? ''}';
                    final amount = t['gross_amount'] ?? 0;
                    final status = t['transaction_status'] ?? '';
                    final time = t['transaction_time']?.toString()?.substring(0,10) ?? '-';
                    return ListTile(
